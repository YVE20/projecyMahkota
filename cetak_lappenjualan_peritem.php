<?php
    ...
    $pdf->Cell(189,7,strtoupper('Laporan Penjualan Peritem'),0,1,'C');
    ...
    $pdf->Cell(189 ,5,'',0,1); //dummy
        ...
        $pdf->Cell(20 ,7,'Diskon (Rp)',1,0,'R');
        $pdf->Cell(20 ,7,'Pajak (Rp)',1,0,'R');
        $pdf->SetFont('Times','B',10);
        $pdf->Cell(20 ,7,'Total (Rp)',1,1,'R');
        ...
        while ($rows = mysqli_fetch_assoc($sql1)) {
            ...
            $pdf->Cell(19 ,8,"Shift ".$rows['shift'],1,0);
            $pdf->Cell(53 ,8,substr($rows['nama'],0,30),1,0);
            $pdf->Cell(20 ,8,uang($rows['harga']),1,0,'R');
            $pdf->Cell(12 ,8,$rows['totaljumlah'],1,0,'C');
            // $pdf->SetFont('Times','',11);
            $pdf->Cell(15 ,8,$rows['satuan'],1,0,'C');
            $pdf->Cell(20 ,8,uang($rows['totaldiskon']),1,0,'R');
            $pdf->Cell(20 ,8,uang($rows['totalpajak']),1,0,'R');
            $pdf->Cell(20 ,8,uang($rows['totalakhir']),1,1,'R');
        ...
        $pdf->Cell(169 ,8,'Total',1,0,'C');
        // set color table footer
        // $pdf->SetTextColor(255, 255, 255);
        // $pdf->SetFillColor(33,150,243);
        // $pdf->Cell(9 ,8,'Rp.',1,0);
        $pdf->Cell(20 ,8,'Rp '.uang($sumtotalakhir),1,1,'R');//end of line
       
        $pdf->Cell(189 ,5,'',0,1); //dummy
        $pdf->Cell(189 ,5,'',0,1); //dummy
 
    $pdf->Output('D','Laporan-Penjualan-Peritem.pdf');
?>